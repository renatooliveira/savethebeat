...
pub fn build_oauth_client(config: &Config) -> BasicClient {
    ...
    let auth_url =
        AuthUrl::new("https://accounts.spotify.com/authorize".to_string()).expect("Invalid auth URL");
...
pub fn store_state(
    ...
    let mut map = store.write().expect("Failed to acquire write lock on state store");
